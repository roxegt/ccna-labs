...
[R1] ────────────          ──────────── [R3]
      Se0/0/0                  Se0/0/1
                 \        /
        10.0.13.0/30 (enlace de respaldo directo R1-R3)
                   \    /
              Se0/0/1  Se0/0/0
...
- La ruta flotante no aparece en `show ip route` mientras la principal esté activa, pero sí aparece en `show running-config` — siempre está configurada, solo que inactiva.
- El valor de AD para la flotante puede ser cualquier número mayor que la ruta principal. AD 5 funciona, pero también es común usar valores como 200 o 210 para dejar margen.
- Este mecanismo es la base del concepto de redundancia en redes — si se combina con un protocolo de routing dinámico, el failover es aún más automático.
